...
single drive. The following outlines how to ensure the EFI partition that
Windows creates isn't ridiculously small, how to remove the Recovery partition
that Windows creates, and how to shrink the Primary partition to make space for
NixOS.
