...
** How to read the tables below:
   ...
   table. The tables below aren't meant to be a cheat sheet as many of the key
   are more nuanced than the description might lead one to believe. I'm
   overlaying the bindings onto my keymap for my Planck keyboard so I can easily
   ponder how to rearrange keys.

 - The table is split in half and heavily uses aliases for the sake of not
...
*** Things of note:
    ...
    - I never use substitute and it's on a home key. I don't even think that's a
      bad habit. I just think substitute is largely has it's use-cases covered
      by more flexible commands.
    ...
    - I really ought to make use of reverse f/t/F/T search, but I haven't made a
      habit out of it.
...
*** Evil functionality keys before / after
  ...
  | functionality | before | after | new mnemonic | Commentary                                    |
  |---------------+--------+-------+--------------+-----------------------------------------------|
  | subst line    | S      |       |              | unbound because not used/useful               |
  | substitute    | s      |       |              | unbound because not used/useful               |
  | undo line     | U      |       |              | unbound because not used/useful               |
  | left          | h      | m     |              | same position movement keys as Qwerty         |
  | down          | j      | n     |              |                                               |
  | up            | k      | e     |              |                                               |
  | right         | l      | i     |              |                                               |
  | end WORD      | E      | F     | far WORD     | foot and forward are other possible mnemonics |
  | end word      | e      | f     | far word     |                                               |
  | find          | f      | s     | search       | right next to till :)                         |
  | rev find      | F      | S     | rev search   |                                               |
  | eol           | $      | I     |              | motion synergy                                |
  | hard bol      | 0      | M     |              | motion synergy                                |
  | insert mode   | i      | l     | ?            | looks like 'i' if you squint                  |
  | insert bol    | I      | L     |              | looks like 'i' if you squint harder           |
  ...
  | next          | n      | h     | ?            | easier to reach but not mnemonic              |
  | prev          | N      | H     | ?            |                                               |
  | redo          | C-R    | U     |              | makes much more sense and easier to do        |
  | scrn mid      | M      | M-i   |              | M-i > M-m due to home key finger rolling      |
  | scrn low      | L      | M-n   |              | motion synergy                                |
  | scrn top      | H      | M-e   |              | motion synergy                                |
  | scroll down   | C-d    | N     |              | motion synergy                                |
  | scroll up     | C-u    | E     |              | motion synergy                                |
  | set mk        | m      | k     |              | mnemonic is still mark for me                 |
  | join lines    | J      | C-j   |              | slightly easier with Esc/C on caps lock key   |
  ...
  | rev repeat    | ,      | "     |              | rev repeat on same key as repeat now          |
  | play macro    | @      | ,     |              | play macro on much easier key to hit          |

**** Visual Mode key remap

     I just couldn't help but remap this on to the home keys because I use it
     all the time and definitely more than I use the replace functionality. In
     addition, notice the finger rolling action from ~r~ to ~s~ / ~t~ (in
     Colemak) to visual/range select till some character. If I left it where it
     was it would have been in a worse position compared to Qwerty, but now it's
     better.
...
*** Left half
  ...
  | shift   |       | Q ex mode    | W next WORD   | F far WORD      | P paste before | B prev WORD   |
  | ======= | ===== | ============ | ===========   | =============== | ============== | ============  |
...
*** Right half
  ...
  | shift   | M hard bol   | N scroll down | E scroll up  | I eol        | O open above  | " rev repeat |
  | Alt     |              | M-n scrn bot  | M-e scrn top | M-i scrn mid |               |              |
  | lower   | { begin para | % goto match  | & reg spec   | } end para   | \vert bol     | Del          |
  | raise   | ^ soft bol   | * next id     | # prev id    | $ eol        | \             | Del          |
  | ======= | ==========   | ============= | ===========  | =========    | ============= | =====        |
  | default | k set mk     | h next        | , play macro | . repeat cmd | / find        | RET/S        |
